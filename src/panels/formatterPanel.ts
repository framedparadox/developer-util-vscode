...
import { assertInputSize, normalizeIndent } from '../limits';
...
export class FormatterPanel {
    ...
    private handleFormatXML(text: string, indent: number) {
        try {
            const formatted = formatXML(assertInputSize(text, 'XML'), normalizeIndent(indent));
    ...
    private handleFormatJSON(text: string, indent: number) {
        try {
            const formatted = formatJSON(assertInputSize(text, 'JSON'), normalizeIndent(indent));
            this._panel.webview.postMessage({
                command: 'formatResult',
                result: formatted,
            });
    ...
    private handleMinifyJSON(text: string) {
        try {
            const minified = minifyJSON(assertInputSize(text, 'JSON'));
            this._panel.webview.postMessage({
                command: 'formatResult',
                result: minified,
            });
    ...
    private handleMinifyXML(text: string) {
        try {
            const minified = minifyXML(assertInputSize(text, 'XML'));
            this._panel.webview.postMessage({
                command: 'formatResult',
                result: minified,
            });
    ...
    private handleFormatSQL(text: string) {
        try {
            const formatted = formatSQL(assertInputSize(text, 'SQL'));
            this._panel.webview.postMessage({
                command: 'formatResult',
                result: formatted,
            });
